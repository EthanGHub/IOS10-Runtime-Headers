//
//     Generated by class-dump 3.5 (64 bit) (Debug version compiled Jul 30 2016 09:41:21).
//
//     class-dump is Copyright (C) 1997-1998, 2000-2001, 2004-2015 by Steve Nygard.
//

#import <objc/NSObject.h>

@class HAPWACScanRequest, NSMutableArray;
@protocol OS_dispatch_queue;

@interface HAPWACScanner : NSObject
{
    _Bool _scanInProgres;
    NSMutableArray *_pendingRequests;
    HAPWACScanRequest *_activeRequest;
    NSObject<OS_dispatch_queue> *_queue;
}

+ (id)sharedInstance;
@property(retain, nonatomic) NSObject<OS_dispatch_queue> *queue; // @synthesize queue=_queue;
@property(retain, nonatomic) HAPWACScanRequest *activeRequest; // @synthesize activeRequest=_activeRequest;
@property(retain, nonatomic) NSMutableArray *pendingRequests; // @synthesize pendingRequests=_pendingRequests;
@property(readonly, nonatomic, getter=isScanInProgress) _Bool scanInProgres; // @synthesize scanInProgres=_scanInProgres;
- (void).cxx_destruct;
- (void)__runRequest:(id)arg1 completionHandler:(CDUnknownBlockType)arg2;
- (void)__nextRequest;
- (void)cancelRequest:(id)arg1 completionHandler:(CDUnknownBlockType)arg2;
- (id)__pendingRequestWithUUID:(id)arg1;
- (void)addRequest:(id)arg1;
- (void)scanForWACAndExtractSetupDictWithNumberOfScans:(unsigned long long)arg1 withScanType:(unsigned long long)arg2 andCompletion:(CDUnknownBlockType)arg3;
- (_Bool)activeScan;
- (void)dealloc;
- (id)init;
- (id)initPrivate;

@end

